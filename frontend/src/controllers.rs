...
impl TaskController {
    ...
    pub fn init_tasks(&self) {
        ...
        wasm_bindgen_futures::spawn_local(async move {
            let fetched_tasks = todo_api::fetched_tasks().await.unwrap();
    ...
    pub fn create_task(&self, title: String) {
        let tasks = self.state.clone();
        wasm_bindgen_futures::spawn_local(async move {
            let response = todo_api::create_task().await.unwrap();
    ...
    pub fn toggle_task(&self, id: String) {
        let tasks = self.state.clone();
        wasm_bindgen_futures::spawn_local(async move {
            let response = todo_api::toggle_task().await.unwrap();
    ...
    pub fn delete_task(&self, id: String) {
        let tasks = self.state.clone();
        wasm_bindgen_futures::spawn_local(async move {
            let response = todo_api::delete_task().await.unwrap();
            if response.rows_affected == 1 {
